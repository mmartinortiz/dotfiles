function azure_remote
    ...
    set --local parsed (echo $origin_url | awk -F'/' '
      /dev\.azure\.com/ {
        company=$(NF-3)
        project=$(NF-2)
        repo=$(NF)
    ...
    set --local company (echo $parsed | awk '{print $1}')
    set --local project (echo $parsed | awk '{print $2}')
    set --local repo (echo $parsed | awk '{print $3}')
    ...
    set --local new_url "https://$pat@dev.azure.com/$company/$project/_git/$repo"
